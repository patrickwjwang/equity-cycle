...
tables_path = os.path.join(current_dir, 'results', 'tables')  # tables directory
graphs_path = os.path.join(current_dir, 'results', 'graphs')  # graphs directory

# Read processed files
ff6_weekly_df = pd.read_csv(os.path.join(processed_path, 'ff6_weekly.csv'))
ff6_monthly_df = pd.read_csv(os.path.join(processed_path, 'ff6_monthly.csv'))
hxz_weekly_df = pd.read_csv(os.path.join(processed_path, 'hxz_weekly.csv'))
hxz_monthly_df = pd.read_csv(os.path.join(processed_path, 'hxz_monthly.csv'))
...
with open(os.path.join(tables_path, 'ff6_weekly_summary.txt'), 'w') as file:
...
# Sore the table in txt format
with open(os.path.join(tables_path, 'hxz_weekly_summary.txt'), 'w') as file:
...
ff6_monthly_df.set_index('Year_Month', inplace=True)
...
for i, var in enumerate(variables):
    axs[i].plot(ff6_monthly_df.index, ff6_monthly_df[var])
    axs[i].set_title(var)
    axs[i].set_xlabel('Date')
    axs[i].set_ylabel('Values')
plt.tight_layout()
os.makedirs(graphs_path, exist_ok=True)
plt.savefig(os.path.join(graphs_path, 'ff6_monthly_plots.png'))


# Line chart for hxz monthly
hxz_monthly_df['Year_Month'] = pd.to_datetime(hxz_monthly_df['Year_Month'])
hxz_monthly_df.set_index('Year_Month', inplace=True)
...
axs = axs.flatten()
for i, var in enumerate(variables):
    axs[i].plot(hxz_monthly_df.index, hxz_monthly_df[var])
    axs[i].set_title(var)
    axs[i].set_xlabel('Date')
    axs[i].set_ylabel('Values')
plt.tight_layout()
os.makedirs(graphs_path, exist_ok=True)
plt.savefig(os.path.join(graphs_path, 'hxz_monthly_plots.png'))
